# BastaSampleGen — .NET Source Generator Template

This template scaffolds a complete .NET source generator solution that includes:

- **`BastaSampleGen/`** – The source generator project (`netstandard2.0`)
- **`BastaSampleGen.Tests/`** – Unit tests using **xUnit v3**
- **`BastaSampleGen.SnapshotTests/`** – Snapshot tests using **xUnit v3** + **Verify**
...
`BastaSampleGenImpl.cs` with your own attribute and code generation.
...
The template ships with two pre-accepted `.verified.txt` files in `BastaSampleGen.SnapshotTests/Snapshots/`, so the included snapshot tests **pass immediately** without any extra setup.
...
Verified snapshots are stored in `BastaSampleGen.SnapshotTests/Snapshots/` and should be
...
1. Open `BastaSampleGenImpl.cs` and replace the `GenerateInfo` attribute name and logic
   with your own.
2. Update `BastaSampleGenTests.cs` and `BastaSampleGenSnapshotTests.cs` with tests for your
   new behavior.
