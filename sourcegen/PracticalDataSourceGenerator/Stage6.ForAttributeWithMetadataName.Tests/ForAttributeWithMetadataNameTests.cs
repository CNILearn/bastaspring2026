...
namespace Stage6.ForAttributeWithMetadataName.Tests;

public class ForAttributeWithMetadataNameDataSourceGeneratorTests
{
    [Fact]
    public void GeneratesDataFactoryForSimpleClassWithoutExternalFiles()
    {
        ...
        Assert.Contains("GetPerformanceStats()", generatedSource);
        
        // Verify Stage 6 characteristics
        Assert.Contains("Stage 6: ForAttributeWithMetadataName Data Source Generator", generatedSource);
        Assert.Contains("Enhanced attribute detection performance", generatedSource);
    ...
    public void GeneratesDataFactoryWithCachedExternalConfiguration()
    {
        ...
        Assert.Contains("GetPerformanceStats()", generatedSource);
        Assert.Contains("ForAttributeWithMetadataName: Optimized attribute detection + cached configuration parsing", generatedSource);
    }
    ...
    public void GeneratesAttributeSourceWithCachingMetadata()
    {
        ...
        Assert.Contains("Stage6.ForAttributeWithMetadataName.Attributes", generatedSource);
        Assert.Contains("Stage 6: ForAttributeWithMetadataName for enhanced performance", generatedSource);
    }

    [Fact]
    public void ForAttributeWithMetadataNameOptimizesAttributeDetection()
    {
        var source = """
            using Stage6.ForAttributeWithMetadataName.Attributes;
        ...
        // Verify ForAttributeWithMetadataName-specific methods are generated
        Assert.Contains("GetPerformanceStats()", generatedSource);
        Assert.Contains("ForAttributeWithMetadataName: Optimized attribute detection", generatedSource);
        
        // Verify Stage 6 branding
        Assert.Contains("Stage 6: ForAttributeWithMetadataName", generatedSource);
        Assert.Contains("enhanced attribute detection", generatedSource);
    }
    ...
    public void CachingHandlesContentHashForChangeDetection()
    {
        ...
        // Verify the generated code indicates ForAttributeWithMetadataName is active
        Assert.Contains("ForAttributeWithMetadataName enabled", generatedSource);
        Assert.Contains("enhanced attribute detection", generatedSource);
    }
}