...
/// <summary>
/// Optimized multi-level cache manager with production-ready performance characteristics
/// Implements lock-free operations where possible, reduces allocations, and provides pluggable cache providers
/// </summary>
internal class OptimizedCacheManager(
    int l1MaxSize = 1000,
    TimeSpan? l1Ttl = null,
    TimeSpan? l2Ttl = null,
    TimeSpan? l3Ttl = null,
    string? persistentCacheDirectory = null)
{
    ...
    private readonly OptimizedPersistentStorage _l2Cache = new(persistentCacheDirectory); // Persistent disk cache
    ...
    public OptimizedCacheStatistics Statistics { get; } = new();
    public OptimizedChangeDetectionEngine ChangeDetection { get; } = new();
    ...
    public async Task PerformMaintenanceAsync()
    {
        await Task.Run(() =>
        {
            // Optimized cleanup using span operations where possible
            PerformL1Maintenance();
            PerformL3Maintenance();
        });
    }

    private void PerformL1Maintenance()
    {
        // Use a pre-allocated list to reduce GC pressure
        const int MaxEntriesPerMaintenance = 100;
        var expiredKeys = new List<string>(MaxEntriesPerMaintenance);

        // Quick pass to identify expired entries
        foreach (var kvp in _l1Cache)
        {
            if (expiredKeys.Count >= MaxEntriesPerMaintenance) break;
    ...
    private void PerformL1Eviction()
    {
    ...
    private void PerformL3Maintenance()
    {
        // Use similar optimized approach for L3 cache
        const int MaxEntriesPerMaintenance = 100;
        var expiredKeys = new List<string>(MaxEntriesPerMaintenance);
    ...
    private void SetL1<T>(CacheEntry<T> entry) where T : class
    {
    ...
    private void PromoteToL1<T>(CacheEntry<T> entry) where T : class
    {
