# HelloGen — .NET Source Generator Template

This template scaffolds a complete .NET source generator solution that includes:

- **`HelloGen/`** – The source generator project (`netstandard2.0`)
- **`HelloGen.Tests/`** – Unit tests using **xUnit v3**
- **`HelloGen.SnapshotTests/`** – Snapshot tests using **xUnit v3** + **Verify**
...
`HelloGenImpl.cs` with your own attribute and code generation.
...
The template ships with two pre-accepted `.verified.txt` files in `HelloGen.SnapshotTests/Snapshots/`, so the included snapshot tests **pass immediately** without any extra setup.
...
Verified snapshots are stored in `HelloGen.SnapshotTests/Snapshots/` and should be
...
1. Open `HelloGenImpl.cs` and replace the `GenerateInfo` attribute name and logic
   with your own.
2. Update `HelloGenTests.cs` and `HelloGenSnapshotTests.cs` with tests for your
   new behavior.
